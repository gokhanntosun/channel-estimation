...
BER_LMS=zeros(length(Ks),length(SNR_dB));%ber matrix
...
for i=1:length(Ks)%pilot count loop
    ...
    for s=1:length(SNR_dB)%snr loop
        ...
        BER_LMS(i,s)=fer/(N*fr);%average over total bits
...
%save results
save('CE_LMS','BER_LMS','MSE_LMS','h_LMS');

figure;
subplot(122);
semilogy(SNR_dB,MSE_LMS(1,:),'r*-');hold on;
semilogy(SNR_dB,MSE_LMS(2,:),'b*-');hold on;
semilogy(SNR_dB,MSE_LMS(3,:),'g*-');hold on;
grid on;legend('P=5','P=10','P=20');title('SNR vs. Avg.MSE');
axis square;set(gca,'FontSize',14);
xlabel('SNR(dB)');ylabel('BER');
subplot(121);
semilogy(SNR_dB,BER_LMS(1,:),'r*-');hold on;
semilogy(SNR_dB,BER_LMS(2,:),'b*-');hold on;
semilogy(SNR_dB,BER_LMS(3,:),'g*-');hold on;
