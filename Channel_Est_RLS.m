...
CE_BER_RLS=zeros(length(Ks),length(SNR_dB));%BERs
...
for i=1:length(Ks)%pilot count loop
    ...
    for s=1:length(SNR_dB)%snr loop
        ...
        while fr<frame_cnt && fer<fer_lim
            ...
            fmse=fmse+mse;%accumulate frame mean squared error
        ...
        CE_BER_RLS(i,s)=fer/(N*fr);%average over total bits
...
%save results
save('CE_RLS','CE_BER_RLS','MSE_RLS','h_RLS');
%helper functions---
